...
function CreateOutpMassForDate(Date, int_mode_y, int_mode_x) {
    ...
    if(int_mode_y == 0) str_mode = "Год издания";
    else if (int_mode_y == 1) str_mode = "Издательство";

    if(int_mode_x == 0) str_mode_date = "Кол-во страниц";
    else if(int_mode_x == 1) str_mode_date = "Кол-во страниц";
    else if(int_mode_x == 2) str_mode_date = "Место в рейтинге";
...
function DrawLinearGrafic_02(data_x, data_y, strX, strY, int_color) {
    ...
    svg.append("g")
        .attr("transform", "translate(" + 27 + ", "+ 230 + ") rotate(-90)")
        .append("text")
        .text(strX)
        .attr("class", "y-legend")
        .style("fill", "gray"); // set color to gray
    ...
    // Указываем цвет осей и легенд
    var axisColor = "#fff";
    var legendColor = "#fff";

    // Выбираем оси и задаем цвет
    svg.selectAll(".axis line")
      .style("stroke", axisColor);
      svg.selectAll(".axis path")
      .style("stroke", axisColor);

    // Выбираем легенды и задаем цвет
...
function DrawGistDiagramm(map, input_b) {    
    ...
    if((input_b == 0) || (input_b == 1)) max = 1500;
    else max = 25;
    ...
    // Задаём степень шакализирования осей
...
function MainGenerateGrafic(input_a, input_b) {
    ...
    if(input_b == 0) strLett = "Max кол-во страниц";
    else if(input_b == 1) strLett = "Min кол-во страниц";
    else if(input_b == 2) strLett = "Рейтинг";
    
    if(input_a == 1) { 
        // Если выбран "Издательство", то рисую линейный график
        DrawGistDiagramm(newDate, input_b);
    } else { 
        // Если выбран "Год издания", то рисую столбчатый график
        DrawLinearGrafic_02(data_x, data_y, strLett, "Издательство", input_b);
    }
}