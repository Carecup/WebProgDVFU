...
function createTable(data) {
  ...
  var headerNames = ["Название", "Автор", "Издательство", "Год издания", "Кол-во страниц", "Место в рейтинге"];
...
function setSortTable(date, strMass) {
  ...
  for(let i = 0; i < date.length; i++) {
    ...
    if(strMass[1] != "") {
      ...
      if (date[i].Автор.indexOf(strMass[1]) === -1) {
        bool = false;
      }
    }
    if(strMass[2] != "") {
      //console.log('strMass[1] = ' + strMass[1] );
      if (date[i].Издательство.indexOf(strMass[2]) === -1) {
        bool = false;
      }
    }
    if(strMass[3] != 0) {
      if(date[i]['Год издания'] != strMass[3]) {
        bool = false;
      }
    }
    if(strMass[4] != 0) {
      if(date[i]['Кол-во страниц'] < strMass[4]) {
        bool = false;
      }
    }
    if(strMass[5] != 0) {
      if(date[i]['Кол-во страниц'] > strMass[5]) {
        bool = false;
      }
    }
    if(strMass[6] != 0) {
      if(date[i]['Место в рейтинге'] < strMass[6]) {
        bool = false;
      }
    }
    if(strMass[7] != 0) {
      if(date[i]['Место в рейтинге'] > strMass[7]) {
        bool = false;
      }
    }
...
allElemMass[2] = 'Автор';
allElemMass[3] = 'Издательство';
allElemMass[4] = 'Год издания';
allElemMass[5] = 'Кол-во страниц';
